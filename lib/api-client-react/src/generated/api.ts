 ...
 */
export const convertDocument = async (conversionInput: ConversionInput, options?: Parameters<typeof customFetch>[1]): Promise<ConversionBatchResult> => {
    const formData = new FormData();
conversionInput.file.forEach(value => formData.append(`file`, value));
if(conversionInput.mode !== undefined) {
 formData.append(`mode`, conversionInput.mode);
 }
if(conversionInput.ocrLanguage !== undefined) {
 formData.append(`ocrLanguage`, conversionInput.ocrLanguage);
 }
if(conversionInput.target !== undefined) {
 formData.append(`target`, conversionInput.target);
 }

  return customFetch<ConversionBatchResult>(getConvertDocumentUrl(),
  {
    ...options,
    method: 'POST'
    ,
...
export const getConvertDocumentMutationOptions = <TError = ErrorType<ErrorResponse>,
    TContext = unknown>(options?: { mutation?:UseMutationOptions<Awaited<ReturnType<typeof convertDocument>>, TError,ConvertDocumentMutationVariables, TContext>, request?: SecondParameter<typeof customFetch>}
...
const {mutation: mutationOptions, request: requestOptions} = options ?
  ...
  return  { mutationFn, ...mutationOptions }}
    ...
    export type ConvertDocumentMutationBody = BodyType<ConversionInput>
    export type ConvertDocumentMutationError = ErrorType<ErrorResponse>
    export type ConvertDocumentMutationVariables = {data: BodyType<ConversionInput>}
